...
class BrainToSentenceEmbeddingModel(B2TModel):
    ...
    def forward(self, batch: B2tSampleBatch) -> ModelOutput:
        ...
        # 2) Temporal mean pooling: [B, T', D] -> [B, D]
        H_TD = H.transpose(1, 2)              # [B, D, T']
        pooled = self.pool(H_TD).squeeze(-1)  # [B, D]

        # 3) Project to semantic embedding space
        pred_emb = self.proj(pooled)          # [B, emb_dim]

        # 4) Supervise with precomputed sentence embeddings
        target_emb = batch.target_embedding   # [B, emb_dim]
        assert target_emb is not None, "target_embedding must be set in batch"
